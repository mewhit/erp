...
const intoResult = effect => Effect.contextWithEffect(context => {
  ...
  Effect.fork, Effect.flatMap(fiber => Effect.onInterrupt(Fiber.join(fiber), () => Fiber.interrupt(fiber))), Effect.interruptible, suspendOnFailure ? Effect.catchAllCause(cause => {
    ...
    onFailure(cause) {
      ...
      return instance.suspended && isInterruptedOnly ? Effect.succeed(new Suspended({
        cause: instance.cause
      })) : !instance.interrupted && isInterruptedOnly || !captureDefects && Cause.isDie(cause) ? Effect.failCause(filtered) : Effect.succeed(new Complete({
...
const withoutInterrupts = cause => Cause.isInterrupted(cause) ? Cause.filter(cause, cause => !Cause.isInterruptType(cause)) : cause;
/**
 * @since 1.0.0
 * @category Result
 */
